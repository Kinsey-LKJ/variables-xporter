...
### 使用 `font` 作为顶层目录
为了让 Design Tokens 的变量结构更加清晰，可以将所有排版和字体相关的变量以 `font/{property}/{variable}` 的格式都放在 `font` 目录下，例如：
- `font/size/sm`
- `font/size/base`
- `font/size/lg`
- `font/weight/normal`
...
## 合并处理 [#merge-property]
你还可以选择将相关的属性以 `font/{variable}/{property}` 的格式组合在一起。这种方式特别适合管理具有固定样式组合的文本相关变量。
...
在以 `font/{variable}/{property}` 的格式合并处理变量时，如果 `{property}` 包含 `font-`前缀，例如 `font-size`、`font-weight`，则可以省略 `font-`。
</Callout>
