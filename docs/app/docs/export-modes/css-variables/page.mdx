import { Cards } from 'nextra/components'
import { Callout } from 'nextra/components'
import { Palette, Type, Layers } from 'lucide-react'

# CSS Variables

选择 CSS Variables 导出方式时，Variables Xporter 会生成一个包含所有 CSS 变量定义的 CSS 文件。这种方式更加简单和灵活，适合不使用 Tailwind CSS 的项目。

## 特点

### 1. 简单直观
直接输出标准的 CSS 自定义属性：

```css
:root {
  --colors-primary: #c03939;
  --colors-primary-foreground: #ffffff;
  --font-size-heading-1: 2.5rem;
  --line-height-heading-1: 1.2;
}
```

### 2. 灵活的使用方式
可以在任何 CSS 规则中使用这些变量：

```css
.button {
  background-color: var(--colors-primary);
  color: var(--colors-primary-foreground);
  font-size: var(--font-size-heading-1);
  line-height: var(--line-height-heading-1);
}
```

### 3. 多模式支持
打造强大的多维变量系统，轻松应对各种设计场景：

```css
/* 基础主题 */
:root {
  --colors-primary: var(--colors-primary-light);
  --spacing-container: var(--spacing-container-default);
  --font-size-base: var(--font-size-base-default);
}

/* 暗色主题 */
.dark {
  --colors-primary: var(--colors-primary-dark);
}

/* 品牌特色 */
.brand-zhiyun {
  --colors-primary: var(--colors-zhiyun-primary);
  --font-family: var(--font-zhiyun);
}
.brand-fiveray {
  --colors-primary: var(--colors-fiveray-primary);
  --font-family: var(--font-fiveray);
}

/* 响应式布局 */
@media (min-width: 768px) {
  :root {
    --spacing-container: var(--spacing-container-md);
    --font-size-base: var(--font-size-base-md);
  }
}
@media (min-width: 1024px) {
  :root {
    --spacing-container: var(--spacing-container-lg);
    --font-size-base: var(--font-size-base-lg);
  }
}

/* 紧凑模式 */
.compact {
  --spacing-base: var(--spacing-base-compact);
  --font-size-base: var(--font-size-base-compact);
}
```

通过[多模式变量管理](/docs/organizing-your-variables/multi-mode)了解如何组织和管理复杂的多模式变量系统，以及查看[多模式支持特性](/docs/features/multi-mode)了解更多使用细节。Variables Xporter 让你能够轻松构建一个支持主题、品牌、响应式、布局密度等多个维度的完整设计系统。

## 变量组织建议

虽然 CSS Variables 方式对变组织的要求较低，但我们仍然建议你遵循良好的组织原则，这样可以：

- 提高变量的可维护性
- 保持设计系统的一致性
- 方便团队协作
- 为将来可能的框架迁移做准备

<Cards>
  <Cards.Card
    icon={<Layers />}
    title="基本原则"
    href="/docs/organizing-your-variables/principles"
  />
  <Cards.Card
    icon={<Palette />}
    title="颜色系统"
    href="/docs/organizing-your-variables/colors"
  />
  <Cards.Card
    icon={<Type />}
    title="排版系统"
    href="/docs/organizing-your-variables/typography"
  />
</Cards>

<Callout type="info" emoji="💡">
  即使你现在不使用 Tailwind CSS，遵循这些组织原则也能让你的设计系统更加规范和专业。如果将来需要迁移到 Tailwind CSS，也会更加容易。
</Callout>       