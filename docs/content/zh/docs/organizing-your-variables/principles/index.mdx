...
<Callout type="info" emoji="ℹ️">
  在导出变量时，无论您使用上述哪种格式，Variables Xporter 都会自动转换输出以匹配 `tailwind.config.js` 文件和 CSS Variables 的约定或要求：
...
<Callout type="info" emoji="ℹ️">
在最新的 Tailwind CSS V4 中，配置文件从 JavaScript 迁移到了 CSS 文件中，因此不再需要使用 `DEFAULT` 关键字，但是我们依然建议使用 `DEFAULT` 关键字来组织变量层级，以确保变量在 Figma 中的层级结构清晰， 当您选择 Tailwind CSS V4 或者 shadcn/ui (Tailwind CSS V4) 作为输出格式时，Variables Xporter 会自动将导出结果中多余的 `DEFAULT` 关键字删除，保证匹配 Tailwind CSS V4 的约定或要求。
</Callout>