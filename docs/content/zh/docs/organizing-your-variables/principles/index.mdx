...
- 使用英文命名：这可以确保与开发环境的兼容性，并在导出到 Tailwind CSS 配置或 CSS 变量时保持一致性
...
您可以在 [Tailwind CSS 主题配置参考](https://tailwindcss.com/docs/theme#configuration-reference) 中找到完整的配置键列表。
...
<Callout type="info" emoji="ℹ️">
  在导出变量时，无论您使用上述哪种格式，Variables Xporter 都会自动转换输出以匹配 Tailwind CSS 配置文件和 CSS 变量的约定或要求：
  - Tailwind CSS 配置文件：使用 camelCase 格式（例如：fontSize, fontWeight）
  - CSS 变量：使用 kebab-case 格式（例如：font-size, font-weight）
</Callout>
...
要找到 `colors/primary` 变量，你需要进入 colors 目录：
...
最棒的是，在实际开发中，你不需要手动写出 `DEFAULT` 关键字。Tailwind CSS 会自动处理这个细节，保持代码简洁：
