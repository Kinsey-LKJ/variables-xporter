...
Variables Xporter offre une fonctionnalité de conversion d'unités, qui peut automatiquement convertir les unités px de Figma en unités rem par défaut de Tailwind CSS.
...
C'est un sujet assez controversé. Bien que Tailwind CSS devienne progressivement la norme pour le développement Web et utilise par défaut l'unité rem, l'utilisation de rem ou de px présente chacune ses avantages et ses inconvénients.
