...
Dans la gestion traditionnelle des Figma Variables, les designers placent généralement toutes les variables de mode dans la même collection :
...
- **Paramètres redondants** : De nombreuses variables ont les mêmes valeurs dans différents modes mais doivent être définies de façon répétée
- **Complexité de gestion** : À mesure que les modes augmentent, les collections deviennent volumineuses et difficiles à gérer
- **Confusion d'utilisation** : Les designers doivent basculer entre plusieurs collections pour trouver les variables appropriées
- **Difficultés d'expansion** : L'ajout de nouvelles dimensions de mode nécessite un travail répétitif considérable
