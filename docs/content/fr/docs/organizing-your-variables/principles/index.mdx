...
<Callout type="info" emoji="ℹ️">
  Lors de l'exportation des variables, quel que soit le format que vous utilisez parmi ceux ci-dessus, Variables Xporter convertira automatiquement la sortie pour correspondre aux conventions ou exigences des fichiers `tailwind.config.js` et des variables CSS :
  - Fichiers `tailwind.config.js` : utilise le format camelCase (par ex., fontSize, fontWeight)
  - Variables CSS : utilise le format kebab-case (par ex., font-size, font-weight)
</Callout>
...
C'est un ensemble courant de variables utilisées pour la couleur primaire et la couleur du texte sur des arrière-plans primaires, mais cette nomenclature seule n'est pas suffisante. Réfléchissons-y en termes de gestion de fichiers :
...
Ne vous inquiétez pas, Tailwind CSS V3 propose une solution élégante — en utilisant le mot-clé `DEFAULT` :
...
Le meilleur, c'est que dans le développement réel, vous n'avez pas besoin d'écrire le mot-clé `DEFAULT`. Tailwind CSS gère automatiquement ce détail, gardant votre code propre :
...
<Callout type="info" emoji="ℹ️">
Dans la dernière version de Tailwind CSS V4, les fichiers de configuration sont passés de JavaScript à CSS, donc le mot-clé `DEFAULT` n'est plus nécessaire. Cependant, nous recommandons toujours d'utiliser le mot-clé `DEFAULT` pour organiser la hiérarchie des variables afin d'assurer une structure hiérarchique claire des variables dans Figma. Lorsque vous choisissez Tailwind CSS V4 ou shadcn/ui (Tailwind CSS V4) comme format de sortie, Variables Xporter supprimera automatiquement les mots-clés `DEFAULT` redondants des résultats exportés pour correspondre aux conventions ou exigences de Tailwind CSS V4.
</Callout>
