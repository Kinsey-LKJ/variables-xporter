...
Dans un Design System, nous utilisons souvent des palettes de couleurs comme variables atomiques, telles que `colors/blue/400`, `colors/red/300`, etc. Pour créer de telles palettes et les utiliser dans l'environnement de développement, vous devez suivre les règles du répertoire de premier niveau, en les plaçant sous le répertoire `colors` ou `color` dans Variables Figma. Par exemple :
...
[^1]: Le mot-clé `DEFAULT` est une façon d'organiser la hiérarchie des variables, [en savoir plus](/docs/organizing-your-variables/principles#using-default-keyword)
