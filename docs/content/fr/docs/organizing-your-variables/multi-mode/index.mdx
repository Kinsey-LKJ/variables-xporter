...
Nous disposons actuellement d'un système de variables qui prend en charge les thèmes clair et sombre. Maintenant, nous voulons aller plus loin et ajouter un système de variables de thème de marque par-dessus les thèmes clair/sombre.
Créez une collection Modes de Marques pour gérer les variables de thème de marque, et créez deux modes de marque différents (dans cet exemple, `zhiyun` et `fiveray`). Ensuite, créez deux variables `colors/primary/light` et `colors/primary/dark`, et définissez différentes valeurs pour elles dans chaque mode (dans cet exemple, en référençant les couleurs pertinentes de la collection principale Design Tokens).
```
 ┌────────────────────────┬──────────────────────┬─────────────────────┐
 │                        │                      │                     │
 ...
 └────────────────────────┴──────────────────────┴─────────────────────┘
```
Ensuite, établissez les relations de référence de variables : les variables dans la collection Modes de Thème référencent les variables correspondantes dans la collection Modes de Marques en fonction du thème actuel, tandis que les variables dans la collection Modes de Marques référencent des valeurs de couleur spécifiques dans la collection Design Tokens. De cette façon, lors du changement de marques ou de thèmes, toutes les couleurs du système se mettront à jour en conséquence.
...
```
Voilà ! Vous avez créé un système de variables multidimensionnel qui inclut à la fois des thèmes clair/sombre et des thèmes de marque, et grâce au principe de point de sortie unique, vous avez rendu votre système de design plus flexible et maintenable. Dans Figma, les utilisateurs n'ont besoin de sélectionner que des variables de la collection Design Tokens, et leurs designs auront à la fois des variables de thème clair/sombre et de thème de marque sans avoir à se soucier de la chaîne de référence des variables.
En examinant la chaîne de référence globale des variables, cela peut sembler quelque peu complexe, mais cette approche est beaucoup plus claire et plus facile à maintenir que la gestion de toutes les variables dans une seule collection. Mais nous n'avons pas encore terminé — développons davantage ce système de variables.
