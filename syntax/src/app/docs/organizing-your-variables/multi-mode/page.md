...
让我们深入探讨如何使用单一出口原则来优雅地组织和管理多模式变量。这种方法不仅能让你的设计系统更加灵活，还能大大提升维护效率。

## 传统的多模式变量组织方式

让我们先来看看传统的多模式变量组织方式。以明暗主题为例，传统做法是在同一个集合中为每个变量设置不同的模式值：
...
现在，我们需要将 Design Tokens 集合中的变量引用到 Density Modes 集合中的变量。这样，当切换到紧凑模式时，所有相关的尺寸都会自动调整。

{% callout title="提示" type="info" %}
**变量引用的优势：** 通过变量引用，我们可以在不同的模式集合中复用变量，避免重复定义。例如，`font/size/2xl` 在默认模式下是 24，但在紧凑模式下变为 22，这种变化通过变量引用轻松实现。

**模式组合的灵活性：** 单一出口原则让我们能够轻松地组合不同的模式。例如，一个变量可以同时受到主题模式、响应式模式和紧凑性模式的影响，而且这种组合是可维护的。
{% /callout %}

## 总结

通过单一出口原则，我们成功构建了一个灵活且可维护的多模式变量系统。这个系统的主要优势包括：

1. **清晰的变量管理**
   - 所有变量都通过主集合（Design Tokens）统一管理
   - 每个模式集合专注于自己的职责
...
记住，好的变量系统不仅要满足当前需求，还要能够适应未来的变化。单一出口原则正是帮助我们实现这一目标的有力工具。
